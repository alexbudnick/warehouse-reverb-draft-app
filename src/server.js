...
async function updateReverbListingPhotos(result, photoUrls) {
  ...
  const updatePath = apiPathFromHref(updateHref);
  ...
  if (!updatePath) {
    throw new Error("Could not determine Reverb update link for photo upload");
  }

  console.log("UPDATING REVERB PHOTOS:");
  console.log(JSON.stringify({ updatePath, photosCount: photos.length, dryRun: DRY_RUN }, null, 2));
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "reverb-dryrun-linkfix-1.2.0"
  });
});
...
app.get("/jobs/warehouse-reverb/create-drafts", requireSecret, async (req, res) => {
  try {
    ...
    for (const record of records) {
      ...
      const photoResult = await updateReverbListingPhotos(result, allPhotos);
