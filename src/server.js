...
function safeImageFilename(filename, index) {
...
function imageProxyUrl(req, attachment, index) {
  const encodedUrl = base64UrlEncode(attachment.url);
  const filename = safeImageFilename(attachment.filename, index);
  return `${publicBaseUrl(req)}/image-proxy/${encodedUrl}/${filename}`;
}

function imageProxyUrls(req, record) {
  const attachments = [
    ...(record.photoAttachments || []),
    ...(record.techPhotoAttachments || [])
  ].filter((attachment) => attachment?.url);

  return attachments.map((attachment, index) => imageProxyUrl(req, attachment, index));
}
...
async function updateReverbListingPhotos(result, photoUrls) {
  ...
  try {
    ...
    return {
      ...
      error: null,
      confirmed: cloudinaryCount === null ? false : cloudinaryCount > 0
    };
  } catch (error) {
...
app.get("/image-proxy/:encodedUrl/:filename", async (req, res) => {
  try {
    const sourceUrl = base64UrlDecode(req.params.encodedUrl);
    const upstream = await fetch(sourceUrl);

    if (!upstream.ok) {
      return res.status(upstream.status).send(`Image fetch failed: ${upstream.status}`);
    }
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "reverb-image-proxy-1.6.0"
  });
});
...
app.get("/jobs/warehouse-reverb/create-drafts", requireSecret, async (req, res) => {
  try {
    ...
    for (const record of records) {
      ...
      const allPhotos = imageProxyUrls(req, record);

      console.log("REVERB PHOTO PROXY URLS:");
      console.log(JSON.stringify({
        sku: record.sku,
        count: allPhotos.length,
        sample: allPhotos[0] || null
      }, null, 2));
