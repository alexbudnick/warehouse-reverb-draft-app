...
async function updateReverbListingPhotos(result, photoUrls) {
  ...
  if (DRY_RUN) {
    ...
    return { uploaded: photos.length, error: null };
  }

  try {
    await reverbPutUrl(updateHref, {
      photos,
      photo_upload_method: "override_position"
    });
    ...
    // Do NOT fail the entire draft creation/writeback because photos can be fixed separately.
    return { uploaded: 0, error: message };
  }
}
...
app.get("/", (req, res) => {
  res.json({
    ...
    version: "reverb-photo-safe-1.3.0"
  });
});
