...
export async function updateWarehouseReverbDraftResult(recordId, result) {
  ...
  const reverbUrlField = process.env.AIRTABLE_REVERB_URL_FIELD || "Reverb URL";
  ...
  const fields = {
    [readyField]: false,
    [readyForGuitarDraftField]: false,
    [readyForGearDraftField]: false,
    [draftSyncSourceField]: "Warehouse Reverb Draft Automation",
    [automationErrorField]: ""
  };

  if (result.reverbListingId) fields[reverbListingIdField] = String(result.reverbListingId);
  if (result.reverbUrl) fields[reverbUrlField] = result.reverbUrl;
