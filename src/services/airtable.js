...
export async function updateWarehouseReverbDraftResult(recordId, result) {
  ...
  const reverbUrlField = process.env.AIRTABLE_REVERB_URL_FIELD || "";
  const automationErrorField = process.env.AIRTABLE_AUTOMATION_ERROR_FIELD || "Automation Error";
  const draftSyncSourceField = process.env.AIRTABLE_DRAFT_SYNC_SOURCE_FIELD || "Draft Sync Source";

  Airtable.configure({ apiKey });
  const base = Airtable.base(baseId);

  // Do NOT clear Ready for Draft / Ready for Guitar Draft / Ready for Gear Draft.
  // Those checkboxes are an audit trail of what the employee clicked.
  const fields = {
    [draftSyncSourceField]: "Warehouse Reverb Draft Automation",
    [automationErrorField]: result.photoUploadError || ""
  };

  if (result.reverbListingId) {
    fields[reverbListingIdField] = String(result.reverbListingId);
  }

  // Only write Reverb URL if a field name is explicitly configured.
  // This avoids UNKNOWN_FIELD_NAME failures in bases without that column.
  if (reverbUrlField && result.reverbUrl) {
    fields[reverbUrlField] = result.reverbUrl;
  }
...
export async function findReadyWarehouseReverbDrafts() {
  ...
  const readyForGuitarDraftField =
    process.env.AIRTABLE_READY_FOR_GUITAR_DRAFT_FIELD || "Ready for Guitar Draft";
  const readyForGearDraftField =
    process.env.AIRTABLE_READY_FOR_GEAR_DRAFT_FIELD || "Ready for Gear Draft";
  ...
  return airtableRecords.map((record) => {
    ...
    const result = {
      ...
      photoAttachments: getAttachmentObjects(fields, "Photos"),
      techPhotoAttachments: getAttachmentObjects(fields, "Tech Photos"),

      photosCount: getAttachmentObjects(fields, "Photos").length,
      techPhotosCount: getAttachmentObjects(fields, "Tech Photos").length
    };
