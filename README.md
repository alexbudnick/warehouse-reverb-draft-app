...
Focused fix:
- Does NOT clear Ready for Draft / Ready for Guitar Draft / Ready for Gear Draft.
- Does NOT require or write `Reverb URL` unless `AIRTABLE_REVERB_URL_FIELD` is explicitly set.
- Writes only Reverb Listing ID, Draft Sync Source, and Automation Error by default.
- Logs Reverb photo update response summary.
- Returns `photosConfirmed` and `photoUploadError` for honest photo reporting.


## Patch Notes 1.6.0

Fixes the actual Reverb photo problem:
- Reverb does not reliably ingest Airtable's extensionless CDN URLs.
- The app now exposes `/image-proxy/[encoded-url]/[filename].jpg` public URLs.
- Reverb receives normal-looking image URLs with real file extensions.
- The proxy streams the Airtable attachment image to Reverb.
- Add Railway variable:
